...
g++ rand31-park-miller-carta.cc -o rand31-pmc && ./rand31-pmc
...
//////////////////////////////////////////////////////////////////////////////
                                    ...
                                    // Test the two implementations by 
                                    // running them through their full 
                                    // cycle of output values - and 
                                    // by checking that their 10,000th
                                    // value is as defined by Park and Miller.

int main ()
{
                                    // Instantiate two random number objects.
    rand31pm ran1pm;
    rand31dc ran2dc;

                                    // Create two 32 bit unsigned integers.
    ...
    cout << "\nComparing the output of a traditional Park Miller PRNG with the output of a\n";
    cout <<   "faster version, which implements David G. Carta's optimisation.\n";
    cout <<   "See http://www.firstpr.com.au/dsp/rand31/\n\n";
        
                                    // Run the loop so it goes just a few 
                                    // than the full 2^32 - 1 cycles.
    
    double  loopcount;
    for(loopcount = 1; loopcount < (double)2147483655.0; loopcount++)
    {
                                    // Get a new value from each 
                                    // generator.
        test1 = ran1pm.ranlui();    
        test2 = ran2dc.ranlui();
        
                                    // Compare the values and spit the 
                                    // dummy (Aussie term for spitting
                                    // out the "pacifier") if they do 
                                    // not agree.
        if( test1 != test2)
        {   
            cout << test1 << " !=  " << test2 << "  Ooooops . . .   loopcount =  " << loopcount <<  "\n";
            exit(0);
        }
        ...
        if(test1 == 1)
        {
            cout << test1 << " ==  " << test2 << " == 1. -------------------------   loopcount = " << loopcount <<  "\n";
        ...
        if(loopcount ==       9997)
        {
            ...
            cout <<   "as predicted by Park and Miller, 1988, page 1195.\n\n";
        }
        else
                                    // Display:
                                    // 
        ...
        if(   (loopcount     <=         10)
            ||(loopcount     >= 2147483640.0)
            ||(   (loopcount >=       9998)
                &&(loopcount <=      10002) 
              ) 
            ||(fmod(loopcount, 1000000) == 0)
          )
        {
            cout << test1 << " ==  " << test2 << "    loopcount = " << (unsigned)loopcount <<  "\n";
        }
